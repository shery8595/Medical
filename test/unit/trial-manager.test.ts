...
import { ARBITRUM_SEPOLIA_CHAIN_ID, MAX_TRIAL_DURATION_SEC } from "../../test-support/constants";
...
describe("Unit: TrialManager", function () {
    ...
    it("TM-02: unverified sponsor reverts on hardhat chain", async function () {
        const stack = await deployMedVaultStack();
        await expectRevert(
            createTrialForSponsor(stack, stack.stranger),
            "SponsorNotVerified"
        );
    });

    it.skip("TM-03: createTrial on Arbitrum Sepolia chain id bypasses sponsor check", async function () {
        // hardhat_setChainId is not available on the default Hardhat network provider.
        const stack = await deployMedVaultStack();
        await ethers.provider.send("hardhat_setChainId", [ARBITRUM_SEPOLIA_CHAIN_ID]);
