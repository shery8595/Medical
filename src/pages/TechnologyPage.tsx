...
export function TechnologyPage() {
  return (
    <div className="flex flex-col bg-slate-50 text-slate-900 min-h-screen overflow-hidden">
      {/* ─── Hero Section ─── */}
      <section className="relative pt-24 pb-16 px-6 lg:px-14 overflow-hidden">
        {/* Background elements */}
        ...
        <div className="relative z-10 max-w-[1220px] mx-auto text-center">
          <motion.div {...fadeIn(0)} className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-teal-100 border border-teal-200 text-[11px] font-bold uppercase tracking-widest text-teal-700 mb-8">
            <CircuitBoard className="h-3.5 w-3.5" />
            Core Infrastructure
          </motion.div>
          <motion.h1 
            {...fadeUp(0.1)}
            className="text-5xl md:text-[5rem] font-bold tracking-tight leading-[1.05] mb-8 text-slate-900"
          >
            The Zenith of <br />
            <span className="text-teal-700">Computational Privacy</span>
          </motion.h1>
          <motion.p 
            {...fadeUp(0.2)}
      ...
      {/* ─── Tech Bento Grid ─── */}
      <section className="py-20 px-6 lg:px-14 relative z-10 bg-slate-100">
        <div className="max-w-[1220px] mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
            ...
            <motion.div 
              {...fadeUp(0)} 
              className="md:col-span-8 group relative rounded-[2.5rem] p-10 lg:p-14 overflow-hidden border border-slate-200 bg-white"
            >
              ...
              <div className="relative z-10">
                ...
                <div className="flex flex-wrap gap-4">
                  {["fhEVM Integration", "Programmable Bootstrapping", "FHE Gates", "Encrypted Booleans", "Bivariate Logic", "Provable Integrity"].map((tag) => (
                    <span key={tag} className="px-4 py-2 rounded-xl bg-slate-100 border border-slate-200 text-xs font-mono text-teal-700">
                      {tag}
                    </span>
                  ))}
                </div>
            ...
            <motion.div 
              {...fadeUp(0.1)} 
              className="md:col-span-4 rounded-[2.5rem] p-10 border border-slate-200 bg-white flex flex-col justify-between"
            >
              <div>
                <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-blue-500/20 text-blue-400 mb-8">
                  <Zap className="h-6 w-6" />
                </div>
                <h3 className="text-xl font-bold mb-4 tracking-tight text-slate-900">Real-time Compute</h3>
              </div>
              <div className="space-y-8">
                <div>
                  <div className="text-4xl font-bold text-slate-900 mb-2 tracking-tight">2.4sec</div>
                  <div className="text-xs uppercase tracking-widest text-slate-500 font-bold">Avg. Eligibility Check</div>
                </div>
                <div className="h-px bg-white/5" />
                <div>
                  <div className="text-4xl font-bold text-slate-900 mb-2 tracking-tight">0kb</div>
            ...
            <motion.div 
              {...fadeUp(0.2)} 
              className="md:col-span-4 rounded-[2.5rem] p-10 border border-slate-200 bg-white"
            >
              <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-emerald-500/20 text-emerald-400 mb-8">
                <ShieldCheck className="h-6 w-6" />
              </div>
              <h3 className="text-xl font-bold mb-4 tracking-tight text-slate-900">Smart Eligibility</h3>
      ...
      <section className="py-24 px-6 lg:px-14 bg-slate-200 relative overflow-hidden">
        <div className="max-w-[1220px] mx-auto text-center relative z-10">
          <h2 className="text-4xl md:text-5xl font-black text-slate-900 mb-8 tracking-tight">Ready to explore the source code?</h2>
          <Button 
            onClick={() => window.location.href = '/docs'}
            className="bg-teal-700 text-white hover:bg-teal-800 h-16 px-12 rounded-2xl text-lg font-bold shadow-xl transition-all hover:scale-105"
          >
