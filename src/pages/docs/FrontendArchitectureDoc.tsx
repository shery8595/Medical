        ...
        <motion.div>
            <Prose className="max-w-none">
                ...
                <p className="lead text-2xl text-slate-500 dark:text-slate-400 mt-6 mb-6 max-w-prose">
                    The MedVault SPA is built using Vite + React 18, using a carefully layered context architecture to manage the complex, asynchronous interplay between MetaMask, the Zama fhEVM coprocessor, and the live GraphQL Subgraph simultaneously.
                </p>
                    ...
                    {contexts.map(ctx => (
                        <div key={ctx.name} className="p-6 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm">
                            ...
                            <div className="p-3 rounded-xl bg-slate-50 dark:bg-slate-950/50 border border-slate-200 dark:border-slate-800">
                                ...
                                <code className="text-xs text-teal-600 dark:text-teal-400">{ctx.exposes}</code>
                            </div>
                        </div>
                    ))}
                </div>
                ...
                <p>
                    FHE transactions take significantly longer to confirm than standard EVM operations due to background polynomial computation in the Zama coprocessor. Every user-facing transaction must move through a well-defined stage machine to prevent UI freeze perception.
                </p>
                ...
                <div className="not-prose flex flex-col sm:flex-row items-stretch gap-3 my-10">
                    {[
                        ...
                        { step: "4", label: "FHE Processing", desc: "Transaction submitted. Zama coprocessor runs FHE (15–60s).", color: "amber", icon: <Loader2 className="w-4 h-4" /> },
