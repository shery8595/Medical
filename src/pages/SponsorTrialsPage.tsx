...
function SummaryStat({
  ...
  return (
    <motion.div
      {...fadeUp(delay)}
      className="group relative overflow-hidden px-6 py-5 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-xl transition-all duration-500 hover:shadow-2xl hover:shadow-teal-500/5 hover:-translate-y-1"
    >
      {/* Glow Layer */}
      <div
      ...
      <div className="relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-8 relative">
          ...
          <Link to="/sponsor/trials/create">
            <button
              className="flex items-center gap-2.5 font-mono text-[11px] font-black uppercase tracking-widest px-6 py-4 rounded-2xl transition-all duration-300 hover:brightness-110 shrink-0 shadow-[0_20px_40px_-10px_rgba(20,184,166,0.3)] bg-teal-600 hover:bg-teal-700 text-white group"
            >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 mb-10">
          <SummaryStat icon={Activity} label="Live Tracks" value={activeCount} iconColor="#14b8a6" delay={0.05} />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-8">
            ...
            <motion.div {...fadeUp(0.2)} className="flex flex-col sm:flex-row gap-3">
              <div className="relative flex-1 group">
                ...
                <input
                  ...
                  className="w-full h-14 pl-12 pr-4 rounded-2xl font-mono text-sm text-slate-200 placeholder-slate-600 focus:outline-none transition-all bg-white/5 border border-white/10 backdrop-blur-md focus:border-teal-500/50 focus:ring-4 focus:ring-teal-500/5"
                  value={searchQuery}
            ...
            <div className="space-y-4">
              {loading ? (
                <div className="py-32 flex flex-col items-center justify-center gap-4">
                  ...
                  >
                    <Sparkles className="h-6 w-6 animate-pulse" style={{ color: "#2dd4bf" }} />
                  </div>
          ...
          <aside className="hidden lg:block space-y-6">
            ...
            >
              <div className="absolute top-0 right-0 w-24 h-24 bg-teal-500/5 blur-3xl rounded-full group-hover:bg-teal-500/10 transition-colors" />
              <h4 className="font-mono text-xs uppercase tracking-[0.25em] text-teal-400 mb-6 font-black">
                Protocol Insights
              </h4>
              <div className="space-y-5">
                <div className="flex justify-between items-center group/item cursor-help">
                  ...
                  <div className="flex gap-1.5">
                    {[1, 2, 3, 4, 5].map((i) => (
                      <div key={i} className={`h-4 w-1 rounded-full transition-all duration-500 ${i <= 4 ? 'bg-teal-500/60 shadow-[0_0_8px_rgba(20,184,166,0.3)]' : 'bg-slate-800'}`} />
                    ))}
                  </div>
                </div>
                <div className="flex justify-between items-center pt-3 border-t border-white/5">
                  ...
                  <div className="flex items-center gap-2">
                    <div className="h-1.5 w-1.5 rounded-full bg-teal-500 animate-pulse shadow-[0_0_8px_rgba(20,184,166,0.8)]" />
                    <span className="text-xs font-black text-teal-400 font-mono uppercase tracking-[0.1em]">Live</span>
                  </div>
                </div>
              </div>
            </motion.div>
            ...
            >
              ...
              <div className="flex items-center gap-3 mb-3">
                ...
                <div className="px-2 py-0.5 rounded-lg text-[9px] font-black bg-teal-500/10 text-teal-400 border border-teal-500/20 shadow-sm">
                  WEEKLY
                </div>
              </div>
            ...
            <motion.div
              {...fadeUp(0.6)}
              className="p-8 rounded-3xl border border-dashed border-white/10 flex flex-col items-center justify-center text-center group cursor-pointer hover:border-teal-500/30 transition-all duration-500 bg-white/[0.02]"
            >
              <Sparkles className="h-6 w-6 text-slate-700 mb-4 group-hover:text-teal-400/50 group-hover:scale-110 transition-all duration-500" />
