...
const VERIFICATION_PATH = "/sponsor/verification";

/** Sponsor routes require wallet + SponsorRegistry verification (unless open-access flag is on). */
export function SponsorGuard({ children }: SponsorGuardProps) {
    ...
    const onVerificationRoute =
        location.pathname === VERIFICATION_PATH || location.pathname.startsWith(`${VERIFICATION_PATH}/`);

    if (!account) {
        return (
            <FullScreenGate>
                <GateIcon icon={<Wallet className="h-7 w-7 text-teal-700" />} />
                ...
                <p className="text-slate-600 text-sm leading-relaxed max-w-sm text-center mb-8">
                    The Sponsor Portal uses Privy: sign in with email or social to get an in-app wallet on Ethereum
                    Sepolia (testnet gas required for transactions).
                </p>
                <Button
                ...
                {connectError ? (
                    <p className="text-rose-600 text-xs max-w-sm text-center mt-4 leading-relaxed">{connectError}</p>
                ) : null}
            </FullScreenGate>
        );
    }
    ...
    if (isLoading) {
        return (
            <FullScreenGate>
                <GateIcon icon={<Loader2 className="h-7 w-7 text-teal-700 animate-spin" />} />
                <p className="mb-3 text-xs font-semibold uppercase tracking-[0.16em] text-teal-700">Sponsor portal</p>
                <h1 className="font-display text-3xl font-semibold text-slate-950 mb-3 tracking-tight">
    ...
    if (!isVerified && !isAdmin) {
        ...
        return <Navigate to={VERIFICATION_PATH} replace state={{ from: location.pathname }} />;
    }
