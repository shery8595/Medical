...
const FAUCET_LINKS: { label: string; href: string }[] = [
  { label: "QuickNode", href: "https://faucet.quicknode.com/arbitrum/sepolia" },
  { label: "Alchemy", href: "https://www.alchemy.com/faucets/arbitrum-sepolia" },
  { label: "Chainlink", href: "https://faucets.chain.link/arbitrum-sepolia" },
];

const dripDevTitle =
  import.meta.env.DEV && !(import.meta.env.VITE_TESTNET_FAUCET_URL as string | undefined)?.trim()
    ? "Dev: POST http://127.0.0.1:8787/drip — run arb-sepolia-faucet (npm start) or set VITE_TESTNET_FAUCET_URL"
    : undefined;
...
export function ArbSepoliaGasBanner() {
  ...
  const requestDrip = async () => {
    ...
    try {
      ...
      setDripTxUrl(explorerUrl || `https://sepolia.arbiscan.io/tx/${txHash}`);
      await refresh();
    } catch (e) {
