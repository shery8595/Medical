...
const EncryptedField: React.FC<{ label: string; delay?: number }> = ({ label, delay = 0 }) => (
  <motion.div
    ...
    className="flex items-center justify-between py-2.5 px-3 rounded-xl bg-slate-800/40 border border-slate-700/40"
  >
    <span className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">{label}</span>
    <div className="flex items-center gap-1.5">
      <Lock className="h-2.5 w-2.5 text-teal-400/70" />
      <span className="text-[10px] font-mono text-teal-400/80 tracking-tight">FHE-SEALED</span>
    </div>
  </motion.div>
);
...
export const VaultCard: React.FC<VaultCardProps> = ({ report }) => {
  ...
  return (
    <motion.div
      ...
      className="group relative overflow-hidden rounded-[2rem] border border-slate-700/50 bg-gradient-to-br from-slate-900 via-slate-900/95 to-slate-950 shadow-2xl shadow-black/30"
    >
      {/* ── Animated background decorations ── */}
      <div className="absolute top-0 right-0 h-72 w-72 -translate-y-1/3 translate-x-1/4 rounded-full bg-teal-500/[0.06] blur-[80px] pointer-events-none transition-all group-hover:bg-teal-500/[0.1]" />
      ...
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top_right,rgba(45,212,191,0.04),transparent_60%)] pointer-events-none" />
      ...
      <div className="relative z-10 p-7">
        ...
        <div className="flex items-start justify-between mb-7">
          <div className="flex items-center gap-4">
            <div className="relative">
              <div className="p-3.5 rounded-2xl bg-gradient-to-br from-teal-500/20 to-teal-600/10 border border-teal-500/20 backdrop-blur-sm">
                <Fingerprint className="h-7 w-7 text-teal-400" />
              </div>
              <motion.div
                animate={{ scale: [1, 1.3, 1], opacity: [0.5, 1, 0.5] }}
                transition={{ repeat: Infinity, duration: 2.5, ease: "easeInOut" }}
                className="absolute -top-1 -right-1 h-3.5 w-3.5 rounded-full bg-emerald-400 border-2 border-slate-900 shadow-[0_0_10px_rgba(52,211,153,0.5)]"
              />
            </div>
            <div>
        ...
        {/* ── Security Pulse Bar ── */}
        <div className="flex items-center justify-between p-3.5 rounded-xl bg-slate-800/50 border border-slate-700/40 mb-6">
          ...
          <div className="flex items-center gap-1">
            {[1, 2, 3, 4, 5, 6, 7, 8].map((i) => (
              <motion.div
                ...
                className="w-[3px] h-4 rounded-full bg-teal-400"
              />
            ))}
          </div>
        </div>
        ...
        <div className="flex items-center justify-between pt-5 border-t border-slate-800/80">
          ...
          <div className="flex gap-2">
            ...
            <button className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-bold text-slate-400 border border-slate-700/50 bg-slate-800/30 hover:text-teal-400 hover:border-teal-500/30 hover:bg-teal-500/5 transition-all">
